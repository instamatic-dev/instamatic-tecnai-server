...
class SimuMicroscope:
    ...
    def __init__(self, name: str = None):
        ...
        self.CurrentDensity_value = 100000.0
        ...
        self.Magnification_value = self._mic_ranges['mag1'][10]
        ##self.Magnification_value_diff = config.microscope.ranges['diff'][3]
        self.Magnification_value_diff = self._mic_ranges['diff'][3]
        ...
        for key in ('a', 'b', 'x', 'y', 'z'):
            if key in ('a', 'b'):
                ...
                speed = 1000000.0  # nm / sec
                current = random.randint(-100000, 100000)
            elif key == 'z':
                speed = 100000.0  # nm / sec
    ...
    def setMagnification(self, value: int):
        ...
        try:
            selector = self._mic_ranges[current_mode].index(value)
        except ValueError:
            raise TEMValueError("No such camera length or magnification: %s" % (value))
            #from None
    ...
    def getMagnificationAbsoluteIndex(self) -> int:
        ...
        if mode in ('mag1', 'samag'):
            n_lowmag = len(self._mic_ranges['lowmag'])
            index += n_lowmag
    ...
    def setMagnificationIndex(self, index: int):
        ...
        if index < 0:
            raise TEMValueError("Cannot lower magnification (index=%s)" % (index))

        try:
            value = self._mic_ranges[current_mode][index]
        except IndexError:
            raise TEMValueError("No such camera length or magnification index: %s" % (index))
            #from None
...
##        return mag_ranges
    ...
    def getMagnificationRanges(self) -> dict:
        ...
        for i, mode in enumerate(self.FUNCTION_MODES):
            try:
                mag_ranges[mode] = self._mic_ranges[mode]
            except ValueError:
                pass
    ...
    def setFunctionMode(self, value: int):
        ...
        if isinstance(value, str):
            try:
                ...
                raise TEMValueError("Unrecognized function mode: %s" % (value))
    ...
    def setScreenPosition(self, value: int):
        ...
        if value not in ('up', 'down'):
            raise TEMValueError("No such screen position: %s ." % (value))
